...
public static class ReflectionUtils
{
	...
	public static IEnumerable<T> GetCustomAttributes<T>(ICustomAttributeProvider type, bool inherit = true) where T : Attribute
	{
		if (type == null)
		{
		...
		foreach (var attrib in type.GetCustomAttributes(typeof(T), inherit))
		{
	...
	public static T GetCustomAttribute<T>(ICustomAttributeProvider type, bool inherit = true) where T : Attribute
	{
		if (type == null)
		{
			throw new ArgumentNullException( "type" );
	...
	public static IEnumerable<MethodInfo> GetAllMethods(Type type, BindingFlags flags)
	{
	...
	public static IEnumerable<PropertyInfo> GetAllProperties(Type type, BindingFlags flags)
	{
		var currentType = type;
	...
	public static MethodInfo GetMethod( Type type, string methodName, Predicate<MethodInfo> predicate )
	{
		...
		if( method == null )
			throw new Exception(string.Format("Method [{0}] not found in [{1}] type or does not satisfy to specified predicate", methodName, type.Name));
